...
void inicializa_vector(void) {
  //VectorInt *aux = (VectorInt) malloc(sizeof(VectorInt *));
  //aux->tam = 0;
  //aux->datos[NUM_DATOS_MAX] = 0;
  int n, r, generated;
  if((n >= 1 || n <= NUM_DATOS_MAX) && (r >= 1 || r <= 1500)){
    printf("Dime el numero de elementos: ");
    scanf("%d", &n);
    printf("\nDime ahora el maximo valor absoluto de los valores a generar: ");
    scanf("%d", &r);
    
    do{
      generated = random_int_max(n);
    }while((generated > 1 || generated < NUM_DATOS_MAX) && (r < ));
  }else
    printf("Error, el rango de los numeros es erroneo.");
}
